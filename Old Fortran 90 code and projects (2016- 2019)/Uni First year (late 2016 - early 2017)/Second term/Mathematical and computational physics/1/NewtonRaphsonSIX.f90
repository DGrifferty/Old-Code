...
INTEGER::i, q, nplusone, n, itwo, ithree, w
...
DO WHILE (fig=='y' .or. fig=='Y')!This do while loop is here so that the user can later run the program again, or exit
  figy = 'n'
  q=1
  IF (figval=='n' .or. figval=='N') THEN !do while loop here so that if the user wants to run the program again, they can use the the same values of a, b, c and d if they want to
    DO WHILE(figy=='n' .or.figy=='N')
     ...
     ELSE IF (figval=='y' .or. figval=='Y') THEN
       WRITE(6,*) 'Using',a,'X^3 +',b,'X^2 +',c,'X +',d !This is here to show users who want to use the same value of a, b, c and d on their secound run that the program is doing so
       END IF
  ...
  DO
    READ(5,*) q
     ...
     END IF 
   END DO
 ALLOCATE(t(q))!Allocates an array to be able to store the amount of roots they want to find, allowing the program to store, and display them all together at the end
 DO i=1,q !Runs main body of the program multiple times so the user can find all the roots that they want to on one run, without having to enter the values again
   IF (i==1) THEN
     root='first' !setting character to allow user to later know what root is being found, thus allowing a nicer interface
     ELSE IF (i==2) THEN
       root='secound'
       ELSE IF (i==3) THEN
         root='third'
   ...
   IF (figtoltwo == 'p' .or. figtoltwo == 'p') THEN !If stops the program writing this message on every run
     ...
     WRITE(6,*) 'must be before the program determines the latter to be a root of the polynomial' !comment here
     DO
       ...
       IF (figtol /= 'n' .and. figtol /='N' .and. figtol /='Y' .and. figtol/='y') then
         ...
         ELSE 
           ...
           END IF
     END DO
     ELSE IF (figtoltwo == 'n' .or. figtoltwo == 'n') THEN !This is implemented, so that if the user choses they are not asked to input their tolerance again
       figtol = 'Y'
     ...
     READ(5,*) xes
     xn=xes !sepearates estimate from number used in iteration so programme can later compare estimate, to final root found
     w=i
     DO itwo=1, 100
            ...
            DO WHILE (figsort == 'p')
              ...
              DO
               ...
               IF(figdevroot == 'y' .or. figdevroot == 'Y') then
                 ...
                 READ(5,*) xes
                 WRITE(6,*)'Would you like to change the deviation tolerance?'!allows user to change deviation tolerance again, if previous value was too sensitive for their function
                 DO
